...
let configure = function(grunt) {
  ...
  let cfg = {
    mochacli: {
      ...
      integration: {
        src: ["test/integration/**/*_spec.js", "test/integration/*_spec.js"]
      },
  ...
  grunt.registerTask(
    "setTestEnv",
    'Use "testing" config unless running on travis',
    () => {
      process.env.NODE_ENV = process.env.TRAVIS
  ...
  grunt.registerTask(
    "test",
    "Run a specific spec file from the tests/ directory",
    test => {
      if (!test) {
  ...
  grunt.registerTask("default", "Run tests", ["test-all"]);
};
